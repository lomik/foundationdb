...
public:
	...
	Future<Reference<IAsyncFile>> readFile(std::string path) {
			return Reference<IAsyncFile>(
				new AsyncFileReadAheadCache(
					Reference<IAsyncFile>(new AsyncFileBlobStoreRead(m_bstore, m_bstore->knobs.bucket, dataPath(path))),
	...
	ACTOR static Future<std::vector<std::string>> listURLs(Reference<BlobStoreEndpoint> bstore) {
		...
		BlobStoreEndpoint::ListResult contents = wait(bstore->listBucket(bstore->knobs.bucket, basePath));
		std::vector<std::string> results;
	...
	Future<Reference<IBackupFile>> writeFile(std::string path) {
		return Reference<IBackupFile>(new BackupFile(path, Reference<IAsyncFile>(new AsyncFileBlobStoreWrite(m_bstore, m_bstore->knobs.bucket, dataPath(path)))));
	}

	Future<Void> deleteFile(std::string path) {
		return m_bstore->deleteObject(m_bstore->knobs.bucket, dataPath(path));
	}
	...
	ACTOR static Future<FilesAndSizesT> listFiles_impl(Reference<BackupContainerBlobStore> bc, std::string path, std::function<bool(std::string const &)> pathFilter) {
		...
		state BlobStoreEndpoint::ListResult result = wait(bc->m_bstore->listBucket(bc->m_bstore->knobs.bucket, bc->dataPath(path), '/', std::numeric_limits<int>::max(), rawPathFilter));
	...
	ACTOR static Future<Void> create_impl(Reference<BackupContainerBlobStore> bc) {
		wait(bc->m_bstore->createBucket(bc->m_bstore->knobs.bucket));

		// Check/create the index entry
		bool exists = wait(bc->m_bstore->objectExists(bc->m_bstore->knobs.bucket, bc->indexEntry()));
		if(!exists) {
			wait(bc->m_bstore->writeEntireFile(bc->m_bstore->knobs.bucket, bc->indexEntry(), ""));
		}
	...
	ACTOR static Future<Void> deleteContainer_impl(Reference<BackupContainerBlobStore> bc, int *pNumDeleted) {
		...
		wait(bc->m_bstore->deleteRecursively(bc->m_bstore->knobs.bucket, bc->dataPath(""), pNumDeleted));

		// Now that all files are deleted, delete the index entry
		wait(bc->m_bstore->deleteObject(bc->m_bstore->knobs.bucket, bc->indexEntry()));
